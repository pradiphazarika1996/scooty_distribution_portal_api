...
// import Cluster from "../../models/masters/Village.model";
...
//       const cachedBlocks = await getApiCache(
//         getCacheKey(API_NAME.BLOCKS, constituency_id),
//       );

//       if (cachedBlocks)
//         return res.status(200).send({ status: true, data: cachedBlocks });
...
//       const cachedClusters = await getApiCache(
//         getCacheKey(API_NAME.CLUSTERS, block_id),
//       );

//       if (cachedClusters)
//         return res.status(200).send({ status: true, data: cachedClusters });
...
//       if (cachedCourses)
//         return res.status(200).send({ status: true, data: cachedCourses });
...
//         return res.status(200).send({ status: true, data: cachedDepartments });
...
//         return res.status(200).send({ status: true, data: cachedDesignations });
...
//       if (cacheStreams)
//         return res.status(200).send({ status: true, data: cacheStreams });
//       const { is_active } = req.query;
